...
async function getContentByLanguage(language) {
  const db = await getDb();
  if (!db) return {};
...
async function updateContent(language, key, value) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const existing = await db.select().from(content).where(and(eq(content.language, language), eq(content.key, key))).limit(1);
  if (existing.length > 0) {
    await db.update(content).set({ value, updatedAt: /* @__PURE__ */ new Date() }).where(and(eq(content.language, language), eq(content.key, key)));
  } else {
...
var appRouter = router({
  ...
  content: router({
    getByLanguage: publicProcedure.input(z2.object({ language: z2.string().default("en") })).query(async ({ input }) => {
      return await getContentByLanguage(input.language);
    }),
    update: protectedProcedure.input(z2.object({ language: z2.string(), key: z2.string(), value: z2.string() })).mutation(async ({ input, ctx }) => {
